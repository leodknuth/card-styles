...
let activeAudioSources = []; // Array to keep track of active audio sources
...
let totalChunks = 0;
let storyChunks = [];
// audioQueue is now managed per-chunk, not globally
...
async function startStoryGeneration() {
  ...
  try {
    ...
    playbackTimeOffset = 0; // Reset offset for new story
    ...
    // 4. Process each chunk sequentially, waiting for each to finish
...
async function preloadChunkAssets(chunkText, index) {
  try {
    ...
    const [imageBlob, audioData] = await Promise.all([
      ...
      prepareAudioData(chunkText)
    ]);

    // Store the preloaded assets
...
// NEW: Main loop to process all chunks sequentially
async function processAllChunks() {
    for (let i = 0; i < totalChunks; i++) {
        // If generation was stopped, exit the loop
        if (!isGenerating) {
        ...
        try {
            await processSingleChunk(currentChunkIndex);
        } catch (chunkErr) {
            console.error(`Failed to process chunk ${currentChunkIndex}. Aborting story.`, chunkErr);
            throw chunkErr; // Propagate error to stop generation
        }
    }
    updateProgress(100);
}
...
// MODIFIED: This function now handles just one chunk and returns a promise
async function processSingleChunk(index) {
  ...
  await waitForChunkPreload(index);

  const preloadedChunk = preloadedChunks[index];

  if (!preloadedChunk || !preloadedChunk.ready) {
      throw new Error(`Chunk ${index} assets are not ready.`);
  }

  // Await the display of image AND the completion of text/audio
  await Promise.all([
      displayPreloadedImage(preloadedChunk.imageBlob, imgContainer),
      displayTextAndPlayAudio(preloadedChunk.text, textContainer, preloadedChunk.audioData)
  ]);
...
// REMOVED: prepareAudio function is superseded by prepareAudioData
// and the new sequential playback logic.

// MODIFIED: This function now returns a Promise that resolves when audio and text are complete.
async function displayTextAndPlayAudio(chunk, textDiv, audioData) {
  return new Promise(async (resolve, reject) => {
    try {
      textDiv.textContent = "";
      ...
      // Reset offset for this specific chunk
      playbackTimeOffset = audioContext.currentTime;

      if (audioData && audioData.length > 0) {
        // Create a promise that resolves when typing is done
        const typingPromise = new Promise(resolveTyping => {
            const totalAudioDuration = audioData.reduce((total, pcmChunk) => {
                return total + (pcmChunk.byteLength / 2) / sampleRate;
            }, 0);
        ...
        const audioPromise = new Promise(resolveAudio => {
            ...
            audioData.forEach((pcmChunk, index) => {
                const source = playAudioChunk(pcmChunk);
                if (index === audioData.length - 1 && source) {
                    source.onended = () => {
                        if (!audioEnded) {
                            audioEnded = true;
                            resolveAudio(); // Audio is complete
                        }
                    };
                }
            });
             // Fallback in case onended doesn't fire (e.g., no audio chunks)
        ...
        // Fallback if no audio: just display the text
...
function playAudioChunk(pcmData) {
  if (!audioContext || pcmData.byteLength === 0) return null;
  ...
  try {
    ...
    if (playbackTimeOffset < now) {
      playbackTimeOffset = now;
    }
    ...
    activeAudioSources.push(source); // Track the source
    source.onended = () => { // Clean up after playing
    ...
    return source; // Return the source to attach onended event
  } catch (err) {
...
async function resetStory() {
    isGenerating = false; // Immediately stop any loops
    ...
    // Close the existing audio context to ensure all sound stops
    if (audioContext && audioContext.state !== "closed") {
        await audioContext.close();
    }
    audioContext = null; // Set to null, will be recreated on next story start
...
function createStars(count) {
  const starsContainer = document.body; // Assuming stars are added to the body
  // Clear existing stars to avoid duplicates on theme change
  document.querySelectorAll('.star').forEach(star => star.remove());
